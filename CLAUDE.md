...
**Data layer.** `arcade_scanner/database/`: `sqlite_store.py` is the main store (`db`), `user_store.py` holds accounts/sessions (PBKDF2). A legacy `video_cache.json` is imported once at startup by `SQLiteStore._migrate_from_json` if present. All data lives in `arcade_data/` (settings.json, SQLite DB, thumbnails). Multi-user isolation: the static HTML dump embeds **no** media entries at all — `window.ALL_VIDEOS` starts empty and is filled at runtime from `/api/videos` (path-filtered per user) and `/api/user/data` (favorite, hidden/vaulted, tags). Earlier versions embedded the entries and stripped the per-user fields; that stripping code was dead by the time it was removed, so don't rely on the dump carrying anything user-specific.
